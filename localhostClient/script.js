      ...
      categories.forEach(category => {
        fetch('https://opentdb.com/api_count.php?category=' + category.id.toString())
          ...
          .then(finalStep => {
            ...
            document.getElementById('checklist').innerHTML += '<div class=\'categories\'>' +
              '<input type=\"checkbox\" id=\"' + category.id + '\" name=\"' + category.name + '\" value=\"' + category.id + '\">' +
              '<label for=\"' + category.id + '\">' + category.name + ' (' + catQNum[catQNum.length - 1].total_question_count + ' total)</label><br>' +
              '<input class=\'diff\' type=\"checkbox\" id=\"easy ' + category.id + '\" name=\"easy\" value=\"easy\">' +
              '<label for=\"' + category.id + ' easy\">Easy Questions (' + catQNum[catQNum.length - 1].total_easy_question_count + ')</label><br>' +
              '<input class=\'diff\' type=\"checkbox\" id=\"medium ' + category.id + '\" name=\"medium\" value=\"medium\">' +
              '<label for=\"' + category.id + ' medium\">Medium Questions (' + catQNum[catQNum.length - 1].total_medium_question_count + ')</label><br>' +
              '<input class=\'diff\' type=\"checkbox\" id=\"hard ' + category.id + '\" name=\"hard\" value=\"hard\">' +
              '<label for=\"' + category.id + ' hard\">Hard Questions (' + catQNum[catQNum.length - 1].total_hard_question_count + ')</label><br>' + 
              '</div>';
          })
      })
    })
...
async function startConnect() {
  ...
  serverUrl = scheme + "://localhost:3000";
